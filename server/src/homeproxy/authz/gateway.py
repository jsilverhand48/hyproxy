...
async def valid_return_url(db: AsyncSession, rd: str) -> bool:
    ...
    resource = await db.scalar(
        select(Resource).where(
            Resource.public_host == parts.hostname, Resource.enabled.is_(True)
        )
    )
...
async def callback(
    ...
    try:
        access = await token_service.verify_access_token(
            db, token=body["access_token"], now=now
        )
...
async def gc_login_states(db: AsyncSession, now: datetime) -> int:
    result = await db.execute(
        delete(GatewayLoginState).where(GatewayLoginState.expires_at <= now)
    )
