...
async def check(body: CheckRequest, db: DbDep) -> CheckResponse:
    ...
    gw = await resolve_gateway_session(
        db, body.gateway_cookie, source_ip=body.source_ip, now=now
    )
    if gw is None:
    ...
    role_rows = (
        await db.execute(
            select(Role.id, Role.name).join(UserRole, UserRole.role_id == Role.id).where(
    ...
    policy_rows = (
        await db.scalars(select(Policy).where(Policy.resource_id == resource.id))
    ).all()
    rules = [
        engine.PolicyRule(
            role_id=p.role_id,
